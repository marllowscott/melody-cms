...
import { getContactPage, STRAPI_URL, type ContactPage } from "@/lib/strapi";
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 text-muted-foreground">
              <a href={`mailto:${pageData?.email1 || 'info@audaxleadership.com'}`} className="hover:text-primary">{pageData?.email1 || 'info@audaxleadership.com'}</a>
              <a href={`mailto:${pageData?.email2 || 'team@audaxleadership.com'}`} className="hover:text-primary">{pageData?.email2 || 'team@audaxleadership.com'}</a>
              <a href={`tel:${pageData?.phone || '+263735402184'}`} className="hover:text-primary">{pageData?.phone || '+263 73 540 2184'}</a>
              <div className="flex gap-4 mt-2">
                <a href={pageData?.facebookUrl || "https://www.facebook.com/MelodyMakuwaza"} target="_blank" rel="noopener noreferrer" className="hover:text-primary">
                  <span className="sr-only">Facebook</span>
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                  </svg>
                </a>
                <a href={pageData?.instagramUrl || "https://www.instagram.com/dialogueswithmel/"} target="_blank" rel="noopener noreferrer" className="hover:text-primary">
                ...
                <a href={pageData?.linkedinUrl || "https://www.linkedin.com/in/melody-chipo-njanji-makuwaza-65b1782a"} target="_blank" rel="noopener noreferrer" className="hover:text-primary">
